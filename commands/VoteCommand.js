    ...
    run(bot, message, args) {
        ...
        message.channel.send("Choissez votre mode de jeux \n \n __**Solo**__ : 🙍 \n __**Duo**__ : 👭 \n __**Squad**__ : 👥").then(msg => {
            ...
            timer:setTimeout(function(){
                var react1 = message.reactions.find(message.emoji.name('🙍')).count;
                var react2 = message.reactions.find(message.emoji.name('👭')).count;
                var react3 = message.reactions.find(message.emoji.name('👥')).count;
                if (react1 > react2) {
                    if(react1 > react3) {
