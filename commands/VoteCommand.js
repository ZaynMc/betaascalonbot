    ...
    run(bot, message, args) {
        ...
        message.channel.send("Choissez votre mode de jeux \n \n __**Solo**__ : 🙍 \n __**Duo**__ : 👭 \n __**Squad**__ : 👥").then(msg => {
            ...
            timer:setTimeout(function(){
                var react1 = message.reactions.find(bot.emoji.name === ':person_with_pouting_face:').count;
                var react2 = message.reactions.find(bot.emoji.name === ':two_women_holding_hands:').count;
                var react3 = message.reactions.find(bot.emoji.name === ':busts_in_silhouette:').count;
                if (react1 > react2) {
                    if(react2 > react3) {
